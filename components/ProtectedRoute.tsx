...
export function ProtectedRoute({ children, requireHost = false, requireAdmin = false }: ProtectedRouteProps) {
  const { isAuthenticated, user, isLoading, isHost, isAdmin } = useAuth();
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      if ((requireHost && !isHost) || (requireAdmin && !isAdmin)) {
  ...
  }, [isAuthenticated, isHost, isAdmin, isLoading, router, requireHost, requireAdmin]);
