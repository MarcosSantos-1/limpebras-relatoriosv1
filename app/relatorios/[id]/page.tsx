...
import type { MutiraoRelatorio, RegistroRelatorio, RevitalizacaoRelatorio, DDSRelatorio, RotineirosRelatorio, Relatorio } from "@/lib/types";
...
export default function RelatorioDetalhePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      let exportType: 'unified' | 'mutirao' | 'evidencias' | 'monumentos' | 'rotineiros' = 'unified';
      
      if (rel.tipoServico === 'MUTIRAO') {
        exportType = 'mutirao';
      } else if (['ACUMULADOR', 'DESFAZIMENTO', 'ALAGAMENTOS', 'ZELADORIA', 'REGISTROS_FOTOGRAFICOS'].includes(rel.tipoServico)) {
        exportType = 'evidencias';
      } else if (rel.tipoServico === 'MONUMENTOS') {
        exportType = 'monumentos';
      ...
      const response = await fetch('/api/generate-pdf', {
        ...
        body: JSON.stringify({ tipo: exportType, dados: rel }),
      });
  ...
  const isRegistro = ["ACUMULADOR", "DESFAZIMENTO", "ALAGAMENTOS", "ZELADORIA", "REGISTROS_FOTOGRAFICOS"].includes(rel.tipoServico);
  const isRevitalizacao = rel.tipoServico === "REVITALIZACAO";
  const isDDS = rel.tipoServico === "DDS";
  const isMonumentos = rel.tipoServico === "MONUMENTOS";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
        <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded p-3">
          ...
          {isMonumentos && (
            <>
              <div><span className="text-zinc-600 dark:text-zinc-400">Data:</span> <span className="font-semibold text-zinc-900 dark:text-zinc-100">{formatDateBR((rel as any).data)}</span></div>
              <div><span className="text-zinc-600 dark:text-zinc-400">Sub:</span> <span className="font-semibold text-zinc-900 dark:text-zinc-100">{(rel as any).sub}</span></div>
              <div><span className="text-zinc-600 dark:text-zinc-400">Setor:</span> <span className="font-semibold text-zinc-900 dark:text-zinc-100">{(rel as any).setorSelecionado}</span></div>
            </>
          )}
          {isRotineiros && (
            <>
      ...
      {isRevitalizacao && (
        <div className="space-y-3">
          <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded p-3">
            ...
            <div className="text-sm text-zinc-700 dark:text-zinc-300 mt-2">{(rel as RevitalizacaoRelatorio).descricao}</div>
